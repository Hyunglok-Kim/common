function VAR_r=Resampling_C(lat_main, lon_main, lat_input, lon_input, VAR, method)
%--------------------------BEGIN NOTE------------------------------%
...
if ~any(~(size(lat_main)==size(lat_input)))
    ...
    if sum(lat_main(:)~=lat_input(:))==0 && sum(lon_main(:)~=lon_input(:))==0
        ...
        VAR_r=resample_mean(lat_main, lon_main, lat_input, lon_input, VAR, method);
    end
else
...
function VAR_r=resample_mean(lat_main, lon_main, lat_input, lon_input, VAR, method)
